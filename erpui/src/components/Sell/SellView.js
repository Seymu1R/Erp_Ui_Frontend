  ...
  useEffect(() => {
    sellservices.getSell(sellId).then(({ data: sell }) => {
      ...
      discountservices.getDiscount(sell.data.discountId).then(({ data: discount }) => {        
        setDiscountName(discount.data.name);
      });
          ...
          <Card>
            ...
            <CardBody CardBody style={{ fontSize: "10px", height: "244px" }}>
              ...
              <Link to={`/sales/update/${sellId}`}>
                <Button
                  style={{ backgroundColor: "#002140", marginTop: "40px" }}
                  color="primary"
                >
                  Edit Sell
                </Button>
        ...
        <Col xs="12" sm="8">
          <Card>
            <CardBody CardBody style={{ fontSize: "25px", height: "500px" }}>
              ...
              <CardText>
                InvoiceStatuse:{" "}
                {sell.invoiceStatuse === 1 ? "Draft" : "Proforma"}
              </CardText>
              ...
              <CardText>Discounts: {discountName}</CardText>
            </CardBody>
          </Card>
        </Col>
      </Row>
